...
function sendRegionData() {
  const sidoText = document.getElementById("sidoButton").innerText;
  const sigunguText = document.getElementById("sigunguButton").innerText;

  if (sidoText !== "시/도 선택" && sigunguText !== "시/군/구 선택") {
    const encodedSido = encodeURIComponent(sidoText);
    const encodedSigungu = encodeURIComponent(sigunguText);

    const url = cpath + "/fundings/ajax?sido=" + encodedSido + "&sigungu=" + encodedSigungu;
    console.log(url);
