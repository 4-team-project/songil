...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/seller_check.css" />
...
<c:when test="${not empty coupon}">
  ...
  <div class="coupon-card-store"><i class="fas fa-store"></i>&nbsp;${store.storeName}</div>
  
  <div class="coupon-card-funding">&nbsp;${funding.fundingName}</div>
  
  <div class="coupon-card-product">&nbsp;${product.productName}</div>
  
  <div class="product-image"><img src="${cpath}/resources/${image[0].imageUrl}" alt="상품 이미지" /></div>
...
<div class="empty-coupon"><strong>잘못된 접근 혹은 쿠폰 정보를 불러오지 못했습니다.</strong>
<br> <strong>다시 확인해주세요.</strong>
</div>
</c:when>
</c:choose>
</body>
