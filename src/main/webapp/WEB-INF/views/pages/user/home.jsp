...
<div class="content-box">
	...
	<p>세연님 <span style="color:#FF9670;">추천 펀딩</span></p>

	<div class="funding-list-wrapper">
		<c:forEach var="funding" items="${fundinglist}" begin="0" end="3">
	<c:set var="percent" value="${(funding.currentQty * 100.0) / funding.targetQty}" />
	<fmt:formatNumber value="${percent}" type="number" maxFractionDigits="0" var="percentInt" />

	<div class="funding-recommend-box">
		<c:choose>
			<c:when test="${not empty funding.images}">
				<img class="funding-recommend-image"
					src="${cpath}/resources/upload/${funding.images[0].imageName}"
					alt="펀딩 이미지">
			</c:when>
			<c:otherwise>
		...
		<div class="funding-contents">
			...
			<div class="funding-progress-box">
				...
				<p>${daysLeftMap[funding.fundingId]}일 남음</p>
			</div>
